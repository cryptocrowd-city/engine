<tr>
    <td>
        <p>
            <?= $vars['translator']->translate('Thank you for your purchase! We sincerely appreciate your support. Your tokens have now been issued.') ?>
        </p>
    </td>
</tr>
<tr>
    <td>
        <p>
            <div><?= $vars['translator']->translate('Date') ?>: <?php echo $vars['date']; ?></div>
            <div><?= $vars['translator']->translate('Token Amount') ?>: <?php echo $vars['amount']; ?></div>
        </p>
    </td>
</tr>
<tr>
    <td>
        <p>
            <?= $vars['translator']->translate('Remember, you can manage your subscriptions and payment methods in your billing settings.') ?>
        </p>
    </td>
</tr>
<tr>
    <td>
        <p>
            <?= $vars['translator']->translate('For any issues, please contact us at') ?>
            <a href="mailto:info@minds.com" <?php echo $emailStyles->getStyles('m-link'); ?>>
                info@minds.com</a>.
        </p>
    </td>
</tr>
